...
export const EditableSelectCell = ({
    ...
    return (
      <div className={`flex items-center space-x-2 ${className}`}>
        ...
        >
          <SelectTrigger 
            ref={selectRef}
            className="h-8 min-w-[120px] text-g01-dark dark:text-white bg-white dark:bg-g01-dark-card border-gray-300 dark:border-gray-600"
            onKeyDown={handleKeyDown}
          >
            <SelectValue placeholder={placeholder} />
          </SelectTrigger>
          <SelectContent className="bg-white dark:bg-g01-dark-card border-gray-200 dark:border-gray-600">
            <SelectItem 
              value="null"
              className="text-g01-gray-medium dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-g01-dark-bg"
            >
              Nenhum
            </SelectItem>
            {options.map((option) => (
              <SelectItem 
                key={option.id} 
                value={option.nome}
                className="text-g01-dark dark:text-white hover:bg-gray-100 dark:hover:bg-g01-dark-bg"
              >
                {option.nome}
              </SelectItem>
            ))}
  ...
  return (
    <button
      ...
      className={`text-left hover:bg-gray-100 dark:hover:bg-g01-dark-bg rounded px-2 py-1 transition-colors min-h-[24px] ${
        loading || isUpdating ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
      } ${className}`}
    >
      <span className="text-g01-gray-medium dark:text-gray-300 font-inter">
        {isUpdating ? (
